...
def descarga_pdb(pdb_ids): # Creo la función para descargar archivos PDBx/mmCIF a partir de una lista de IDs.
    for pdb_id in pdb_ids:
        ...
        if response.status_code == 200:  # Verificar si la descarga fue bien.
            ...
            print(f"Archivo descargado y guardado: '{output_path}'.")  # Confirmar que la descarga fue exitosa.
        else:
            print(f"Error al descargar el archivo: {pdb_id}")  # Aviso si hay  error en la descarga.
...
print("Cofactor encontrado:", cofactor_nombre)  # Imprimo el nombre del cofactor encontrado
...
heteromoleculas = []  # Creo una lista vacía para almacenar los nombres de las heteromoléculas encontradas (excluyendo el agua).
    ...
    for chain in model:  
        for residue in chain:
            ...
            if residue.id[0] != ' ' and residue.resname != 'HOH':  
                heteromoleculas.append(residue.resname)  # Agrego el nombre del residuo a la lista de heteromoléculas.


print("Heteromoléculas encontradas (sin incluir agua):", heteromoleculas)  # Muestra la lista de heteromoléculas encontradas.
...
# Extraigo información sobre las heteromoléculas:
...
df_heteromoleculas = pd.DataFrame({  # Defino el DataFrame con dos columnas: 'Nombre' y 'ID_3Letras'.
    'Nombre': nonpoly_names,  
    'ID de tres letras': nonpoly_ids 
})

# Para excluir el agua
df_heteromoleculas = df_heteromoleculas[df_heteromoleculas['ID de tres letras'] != 'HOH']
...
# Creo un DataFrame a partir de la lista de heteromoléculas sin duplicados y con las columnas SMILES y SDF:
df_heteromoleculas_sinduplicados = pd.DataFrame({'ID_3Letras': list(set(heteromoleculas))})
df_heteromoleculas_sinduplicados['SMILES'] = None
df_heteromoleculas_sinduplicados['SDF'] = None
...
# Recorrer cada heteromolécula en el DataFrame para obtener SMILES y convertirlo en SDF
for index, row in df_heteromoleculas_sinduplicados.iterrows():
    ...
    if smiles:  
        df_heteromoleculas_sinduplicados.at[index, 'SMILES'] = smiles  # Almaceno el SMILES en el DataFrame.
        mol = Chem.MolFromSmiles(smiles)  # Convierto el SMILES a un objeto de molécula.
        if mol:  # Verifico si se creó la molécula.
            AllChem.Compute2DCoords(mol)  # Calculo las coordenadas 2D.
            sdf = Chem.MolToMolBlock(mol)  # Convierto la molécula a formato SDF.
            df_heteromoleculas_sinduplicados.at[index, 'SDF'] = sdf  # Almaceno el SDF en el DataFrame.
        else:
            print(f"No se pudo convertir SMILES a molécula para {nombre}.")  # Mensaje si falla la conversión.

print(df_heteromoleculas_sinduplicados)  # Imprimo el DataFrame con la información actualizada.



# %%
# D
df_heteromoleculas_sdf = df_heteromoleculas_sinduplicados.dropna(subset=['SDF']) # Para filtrar e incluir solo las moleculas que tienen SDF
# Creo archivo .sdf:
with Chem.SDWriter("heteromolecules.sdf") as sdf_writer:
    for index, row in df_heteromoleculas_sinduplicados.iterrows():
        try: #Convertir el SMILES a un objeto molecula usando RDKit
            ...
            if mol:
                mol.SetProp("Peso Molecular", str(AllChem.CalcExactMolWt(mol)))
...
for mol in supplier:
    if mol is not None:
        if mol.HasProp("Peso Molecular"):
            print(Chem.MolToMolBlock(mol))
            print('-------------------')
            print("Peso Molecular:", mol.GetProp("Peso Molecular"))
        else:
            print("Error: 'Molecular_weight' no encontrado en la molécula.")
    else:
        print("Molécula inválida encontrada en el archivo SDF.")
